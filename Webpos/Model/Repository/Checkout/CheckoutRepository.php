...
class CheckoutRepository implements \Bkademy\Webpos\Api\Checkout\CheckoutRepositoryInterface
{
    ...
    /**
     * @var \Bkademy\Webpos\Api\Data\Checkout\ShippingDataInterface
     */
    protected $shippingData;

    /**
     * @var \Bkademy\Webpos\Api\Data\Checkout\PaymentDataInterfaceFactory
     */
    ...
    public function __construct(
        ...
        \Bkademy\Webpos\Api\Data\Checkout\ShippingDataInterfaceFactory $shippingData,
        \Bkademy\Webpos\Api\Data\Checkout\PaymentDataInterfaceFactory $paymentData,
...
//       \Zend_Debug::dump( $this->_getQuoteItems());die;
    ...
    public function placeOrder($quoteId){
        ...
        if(!$order){
            throw new \Magento\Framework\Exception\LocalizedException(__('Lỗi.'));
        }
//        die(json_encode([$order->getId()]));
//        die(json_encode([1,2,$order]));
...
//        die(json_encode([$totalsResult]));
    ...
    protected function _getShipping(){
        ...
        foreach ($shippingRates as $carrierRates) {
            foreach ($carrierRates as $rate) {
                ...
                $shipping = $this->shippingData->create()->setCode($methodCode)->setTitle($methodTitle)->setPrice($methodPrice);
    ...
    protected function _getPayment(){
        ...
        foreach ($methods as $method) {
            $payment = $this->paymentData->create()->setCode($method->getCode())->setTitle($method->getTitle());
            $paymentList[] = $payment;
        };
        return $paymentList;
    }
